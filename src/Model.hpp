...
public:
  unsigned int TextureFromFile(const char *path, const string &directory) {
    ...
    // ChatGPT wrote this.
    // --- Path fix begin ---
    // If the model references an absolute path (like
    // D:\3D\Characters\texture.jpg) and that file doesn’t exist locally, try
    // resolving it relative to the model folder.
    std::ifstream fileCheck(filename);
    if (!fileCheck.good()) {
      // Extract just the filename
      size_t pos = filename.find_last_of("/\\");
      string justName =
          (pos == string::npos) ? filename : filename.substr(pos + 1);
